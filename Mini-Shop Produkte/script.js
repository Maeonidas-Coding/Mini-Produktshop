...
function buildFiltersDef(rows) {
  const map = {};
  for (const r of rows) {
    const field = norm(r.FilterTyp ?? r.Field ?? r.field ?? r.Feld);
    const value = norm(r.FilterWert ?? r.Value ?? r.value ?? r.Wert);
...
function productCardHTML(p) {
  ...
  return `
    <div class="border border-gray-200 rounded-lg p-3 flex flex-col md:flex-row md:items-stretch md:justify-between gap-3">
      <div>
...
// Danke für's reinschauen in meinen Code, Maeonidas lässt grüßen
...
function renderServices() {
  ...
  const q = toLower(byId('search-services')?.value);
  if (q) {
    shown = shown.filter(s =>
      toLower(s.name).includes(q) ||
      toLower(s.id).includes(q) ||
...
function renderCart() {
  ...
  list.innerHTML = entries.map(([pid, {qty}], idx) => {
    ...
    return `
      <div class="border border-gray-200 rounded-lg p-3 flex flex-col md:flex-row md:items-stretch gap-3">
        <!-- Produktinfos -->
        ...
        <!-- Buttons: Info / Entfernen -->
        <div class="flex justify-center w-40 gap-2">
           <button class="flex-1 px-3 py-2 rounded bg-slate-700 text-white hover:bg-slate-600" data-action="info" data-id="${pid}">Info</button>
            <button class="flex-1 px-3 py-2 rounded bg-rose-600 text-white hover:bg-rose-700" data-action="remove" data-id="${pid}">Entfernen</button>
        </div>

        <!-- Menge -->
        <div class="flex items-center justify-center w-32 gap-2">
...
function applyLoadedData({ filters = [], products = [], services = [] }) {
  ...
  state.products = products.map(r => {
    const obj = { ...r };
   obj.id   = norm(r.ProduktID ?? r.id ?? r.ID ?? r.Id);
    obj.name = norm(r.Produktname ?? r.name ?? r.Name ?? r.Produkt ?? r.Product);
    obj.price = coerceNumber(r.Preis ?? r.price ?? r.Price, 0);
    obj.service_ids = parseServiceIds(r.ServiceIDs ?? r.service_ids ?? r.services ?? r['Service IDs'] ?? r['ServiceIds']);

// Extra: Projektspezifische Felder auch ins Objekt übernehmen
    obj.Projektbetrieb = norm(r.Projektbetrieb);
    obj.Geschäftsvorfall = norm(r.Geschäftsvorfall);
     return obj;
  });

  // Services
  state.services = services.map(r => ({
    id: norm(r.ServiceID ?? r.id ?? r.ID ?? r.Id),
    name: norm(r.ServiceName ?? r.name ?? r.Name ?? r.Service),
    description: norm(r.Beschreibung ?? r.description ?? r.beschreibung ?? '')
  }));
